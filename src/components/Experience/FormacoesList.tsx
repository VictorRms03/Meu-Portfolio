...
export default function FormacoesList() {
    const formacoes = [
        { nome: "Bacharelado em Ciência da Computação - Instituto Federal de São Paulo (IFSP)", data: "Mar 2022 - Presente (Previsão: Jan 2026)" },
        { nome: "Técnico em Informática para Internet - Escola Técnica Estadual (ETEC)", data: "Jan 2019 - Dez 2021" },
    ];

    return (
        <div className="max-w-9/12 mx-auto gap-6 py-12 ">
            {formacoes.map((formacao, index) => (
                <GerarFormacao key={index} formacao={formacao.nome} data={formacao.data} />
            ))}
        </div>
    )
}
